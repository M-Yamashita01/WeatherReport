class LocationOnForecastDay < ApplicationRecord
  ...
  scope :search, lambda { |params|
    search_by_date(params[:date]).search_by_main_city_flag(params[:main_city_flag]).search_by_less_than_longitude(params[:longitude_max]).search_by_more_than_longitude(params[:longitude_min]).search_by_less_than_latitude(params[:latitude_max]).search_by_more_than_latitude(params[:latitude_min])
  }
